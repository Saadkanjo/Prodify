...
namespace api.Service
{
  ...
  private readonly IConfiguration _config;
        ...
        public TokenService(IConfiguration config)//CTORc
        {
            _config = config;
            _key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_config["JWT:SigningKey"]));//we use encoding to turn it into bytes (does not accept it as a string)
        }
